  ...
  return (
    <article className="phase-card">
      ...
      <p className="phase-copy">
        Confirm the interpreted topic, curate the shortlist, and accumulate approved
        papers before moving into analysis.
      </p>
            ...
            <form onSubmit={handleApprovedSubmit}>
              <div className="paper-grid">
                {snapshot.latest_shortlist.map((paper) => {
                  ...
                  return (
                    <label className={`paper-card ${isSelected ? "paper-card-selected" : ""}`} key={paper.paper_id}>
        ...
        <section className="action-panel">
          <div className="panel-header">
            ...
            <span className="helper-copy">
              include {snapshot.steering_preferences.include.length} - exclude {snapshot.steering_preferences.exclude.length} - emphasize {snapshot.steering_preferences.emphasize.length}
            </span>
          </div>
          <div className="token-group">
            {snapshot.steering_preferences.include.map((value) => (
              <span className="token include-token" key={`include-${value}`}>
                include: {value}
              </span>
            ))}
            {snapshot.steering_preferences.exclude.map((value) => (
              <span className="token exclude-token" key={`exclude-${value}`}>
                exclude: {value}
              </span>
            ))}
            {snapshot.steering_preferences.emphasize.map((value) => (
              <span className="token emphasize-token" key={`emphasize-${value}`}>
                emphasize: {value}
              </span>
            ))}
            ...
            <ul className="approved-list">
              {approvedPapers.map((paper) => (
                <li key={paper.paper_id}>
                  ...
                  <span>{paper.year ?? "Unknown year"} - {paper.paper_id}</span>
                </li>
              ))}
            </ul>
          ) : (
