...
export function AnalysisView({
  ...
  const edgeLines = useMemo(() => {
    if (!snapshot.citation_graph) return [];
          ...
          <>
            {overCap && (
              <p className="inline-note">
                More than {ANALYSIS_CAP} papers approved — select a subset before starting.
              </p>
            )}
              ...
              return (
                <article className="analysis-paper-card" key={analysis.paper_id}>
                  <div className="panel-header">
                    ...
                    <span className={`quality-badge ${degraded ? "quality-badge-degraded" : "quality-badge-full"}`}>
      ...
      {snapshot.citation_graph && (
        <section className="action-panel">
          <div className="panel-header">
            ...
            <span className="helper-copy">
              {seedNodes.length} seed &nbsp;·&nbsp; {contextNodes.length} context
            </span>
          </div>
          ...
          <div className="graph-wrap">
            <svg viewBox="0 0 360 260" className="graph-canvas" role="img" aria-label="Citation graph">
              ...
              {seedNodes.map((node, index) => (
                <g key={node.node_id} transform={`translate(${graphPositions.seedX(index)}, ${graphPositions.seedY})`}>
                  ...
                  <text y="5" textAnchor="middle" className="graph-label">S{index + 1}</text>
                </g>
              ))}
              {contextNodes.map((node, index) => (
                <g key={node.node_id} transform={`translate(${graphPositions.contextX(index)}, ${graphPositions.contextY})`}>
                  <rect x="-18" y="-18" width="36" height="36" rx="4" className="graph-node graph-node-context" />
                  <text y="5" textAnchor="middle" className="graph-label">C{index + 1}</text>
                </g>
              ))}
            </svg>
            <div className="graph-legend">
              ...
              <ul className="compact-list">
                {seedNodes.map((node, index) => (
                  <li key={node.node_id}>S{index + 1}: {node.title}</li>
                ))}
              </ul>
              <h4>Context</h4>
              <ul className="compact-list">
                {contextNodes.map((node, index) => (
                  <li key={node.node_id}>C{index + 1}: {node.title}</li>
                ))}
              </ul>
            </div>
          </div>
